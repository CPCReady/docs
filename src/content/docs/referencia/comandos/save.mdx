...
**Conversión automática a formato DOS:** Los archivos se convierten automáticamente a formato DOS (líneas terminadas en CRLF) antes de guardarlos, asegurando compatibilidad total con emuladores CPC.
...
- **a** (ASCII): Archivo ASCII plano/datos
  ...
  - Convertido automáticamente a formato DOS (CRLF)

## Conversión de Formato de Línea

CPCReady convierte automáticamente todos los archivos a formato DOS antes de guardarlos en el disco DSK. Esto garantiza:

- **Compatibilidad con CPC**: Los archivos BASIC ASCII y de texto se pueden cargar correctamente en emuladores
- **Comportamiento idéntico a iDSK**: Los archivos guardados son byte por byte idénticos a los generados por iDSK
- **Terminaciones de línea CRLF**: Los archivos Unix (LF) se convierten automáticamente a DOS (CRLF)
- **Proceso transparente**: La conversión ocurre automáticamente sin intervención del usuario

<Aside type="note">
La conversión de formato se aplica a todos los archivos, independientemente del tipo especificado. Los archivos temporales creados durante la conversión se eliminan automáticamente después del guardado.
</Aside>
