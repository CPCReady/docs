...
var convertItpToXyz65 = ({ i, t, p: p3, alpha }) => {
  if (i === void 0) i = 0;
  if (t === void 0) t = 0;
  if (p3 === void 0) p3 = 0;
  const l = transferPqDecode(
    i + 0.008609037037932761 * t + 0.11102962500302593 * p3
  );
  const m = transferPqDecode(
    i - 0.00860903703793275 * t - 0.11102962500302599 * p3
  );
  const s = transferPqDecode(
    i + 0.5600313357106791 * t - 0.32062717498731885 * p3
  );
  const res = {
    mode: "xyz65",
    x: toRel(
...
var convertXyz65ToItp = ({ x, y, z, alpha }) => {
  ...
  const p3 = 4.378173828125 * l - 4.24560546875 * m - 0.132568359375 * s;
  const res = { mode: "itp", i, t, p: p3 };
  if (alpha !== void 0) {
    res.alpha = alpha;
  }
  return res;
...
var convertLchuvToRgb = (lchuv) => convertXyz50ToRgb_default(convertLuvToXyz50_default(convertLchuvToLuv_default(lchuv)));
var definition17 = {
  mode: "lchuv",
  toMode: {
...
var definition19 = {
  ...
  toMode: {
    ...
    rgb: (luv) => convertXyz50ToRgb_default(convertLuvToXyz50_default(luv))
  },
  fromMode: {
...
var mono = (arr) => {
  ...
  for (let i = 0; i < n; i++) {
    ...
    p3.push(i > 0 ? 0.5 * (arr[i + 1] - arr[i - 1]) * n : void 0);
    yp.push(
      i > 0 ? (sgn(s[i - 1]) + sgn(s[i])) * min(abs2(s[i - 1]), abs2(s[i]), 0.5 * abs2(p3[i])) : void 0
    );
  }
  return [s, p3, yp];
};
...
var interpolatorSplineMonotone2 = (arr) => {
  ...
  let n = arr.length - 1;
  let [s, p3, yp] = mono(arr);
  p3[0] = (arr[1] * 2 - arr[0] * 1.5 - arr[2] * 0.5) * n;
  p3[n] = (arr[n] * 1.5 - arr[n - 1] * 2 + arr[n - 2] * 0.5) * n;
  yp[0] = p3[0] * s[0] <= 0 ? 0 : abs2(p3[0]) > 2 * abs2(s[0]) ? 2 * s[0] : p3[0];
  yp[n] = p3[n] * s[n - 1] <= 0 ? 0 : abs2(p3[n]) > 2 * abs2(s[n - 1]) ? 2 * s[n - 1] : p3[n];
  return interpolator(arr, yp, s);
};
var interpolatorSplineMonotoneClosed = (arr) => {
  let n = arr.length - 1;
  let [s, p3, yp] = mono(arr);
  p3[0] = 0.5 * (arr[1] - arr[n]) * n;
  p3[n] = 0.5 * (arr[0] - arr[n - 1]) * n;
  let s_m1 = (arr[0] - arr[n]) * n;
  let s_n = s_m1;
  yp[0] = (sgn(s_m1) + sgn(s[0])) * min(abs2(s_m1), abs2(s[0]), 0.5 * abs2(p3[0]));
  yp[n] = (sgn(s[n - 1]) + sgn(s_n)) * min(abs2(s[n - 1]), abs2(s_n), 0.5 * abs2(p3[n]));
  return interpolator(arr, yp, s);
};
...
//# sourceMappingURL=culori_fn.js.map
